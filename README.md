...
### How to start app in localhost:
1. Install django 
```$ pip install django```
2. On Linux or OS X, you should probably install Django in an isolated virtualenv:
* ```$ virtualenv -p python3 ~/djangoenv```
* ```$ source ~/djangoenv/bin/activate```
* ```$ pip install django```
4. cd into the directory
3. Activate virtual environment on Linux or OS X:
```$ source ~/djangoenv/bin/activate```
